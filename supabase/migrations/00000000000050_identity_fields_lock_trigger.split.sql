-- Safer / split apply for identity lock (run ONE section at a time).
-- If you get deadlock: wait a few seconds, retry that section only.
-- Prefer running while the app has little traffic.

-- ========== 1) Audit column (light) ==========
...
-- ========== 2) Trigger function (no table lock) ==========
...
-- ========== 3) Triggers on clients (needs brief table lock) ==========
-- Close other SQL tabs first. Retry if deadlock.
set lock_timeout = '5s';
...
-- ========== 5) Correction RPC (no table lock) ==========
