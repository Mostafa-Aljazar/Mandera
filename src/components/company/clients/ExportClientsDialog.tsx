...
} from "@/lib/importExport/clientExportColumns";
import type { ClientWithRelations } from "@/types/supabase-entities.types";

interface ExportClientsDialogProps {
  isOpen: boolean;
  onClose: () => void;
  rows: ClientWithRelations[];
  /** How many clients are currently checked in the list — 0 means none, so all filtered clients export. */
...
export default function ExportClientsDialog({
  isOpen,
  onClose,
  rows,
  selectedCount,
  ...
  const [selectedKeys, setSelectedKeys] = useState<Set<string>>(
    () => new Set(CLIENT_EXPORT_COLUMNS.filter((c) => c.defaultSelected).map((c) => c.key)),
  );
  const [exportLanguage, setExportLanguage] = useState(language === "ar" ? "ar" : "en");
  const [isExporting, setIsExporting] = useState(false);

  const selectedColumns = useMemo(
    () => CLIENT_EXPORT_COLUMNS.filter((c) => selectedKeys.has(c.key)),
    [selectedKeys],
  );
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      const ctx = { language: exportLanguage, t, employees, statuses };
      ...
      <DialogContent className="sm:max-w-[520px]">
        ...
        <div className="space-y-4 py-2">
          <div className="flex items-start gap-2 rounded-lg border border-primary/20 bg-primary/5 p-3 text-xs text-foreground">
            ...
            <span>
              {selectedCount > 0
                ? t(
                    "{{count}} client(s) are selected in the list — only those will be exported.",
                    { count: selectedCount },
                  )
                : t(
                    "No clients are selected, so all {{count}} client(s) currently shown will be exported. Check specific clients in the list first to export just those.",
                    { count: rows.length },
                  )}
            </span>
          </div>
              ...
              <div className="space-y-2.5">
                {CLIENT_EXPORT_COLUMNS.map((col) => (
                  <div key={col.key} className="flex items-center gap-2.5">
                    ...
                    />
                    <Label
                      htmlFor={`client-col-${col.key}`}
                      className="text-sm font-normal cursor-pointer"
                    >
                      {col.getHeader({ language, t, employees, statuses })}
                    </Label>
                  </div>
                ))}
              </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={selectedColumns.length === 0 || rows.length === 0 || isExporting}
            className="gap-2"
          >
            {isExporting ? (
