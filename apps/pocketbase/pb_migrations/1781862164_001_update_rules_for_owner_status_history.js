...
migrate((app) => {
  const collection = app.findCollectionByNameOrId("owner_status_history");
  collection.createRule = "@request.auth.companyId = company_id && (@request.auth.role = 'company_super_admin' || @request.auth.id = owner_id.assigned_employee_id)";
  ...
  try {
  const collection = app.findCollectionByNameOrId("owner_status_history");
  collection.createRule = "@request.auth.companyId = company_id && (@request.auth.role = 'company_super_admin' || (@request.auth.id = created_by && @request.auth.id = owner_id.assigned_employee_id))";
