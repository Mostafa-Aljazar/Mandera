...
export const useEmployeeDeletion = () => {
  ...
  const deleteEmployeeWorkflow = async (employeeToDelete, { reassignOwnersTo, reassignClientsTo, reassignPropertiesTo }) => {
    setIsDeleting(true);
    setDeletionProgress('');
    setDeletionError('');
    ...
    try {
      ...
      const errorMessage = error.message || t('An error occurred during the process');
