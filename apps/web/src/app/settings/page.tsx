...
import type { PropertyType, ClientStatus, CompanyEmployee } from '../../../types/pocketbase.types';
...
const SettingsPage = () => {
  ...
  const [editItem, setEditItem] = useState<PropertyType | ClientStatus | null>(null);
  const [formData, setFormData] = useState<SettingsFormData>({ name: '', priority_order: 1 });
  ...
  const fetchData = async () => {
    ...
    try {
      const [ptData, csData, empData] = await Promise.all([
        ...
        pb.collection('client_statuses').getFullList<ClientStatus>({ filter: `company_id = "${company.id}"`, sort: 'priority_order,created', $autoCancel: false }),
  ...
  const handleSave = async (collection: string, setOpen: (open: boolean) => void) => {
    if (!formData.name.trim()) return;

    const payload: { name: string; company_id: string; priority_order?: number } = { name: formData.name, company_id: company!.id };

    if (collection === 'client_statuses') {
      if (!formData.priority_order || Number(formData.priority_order) < 1) {
        toast.error(t('Priority order must be a positive number.'));
        return;
      }
      payload.priority_order = parseInt(formData.priority_order as string, 10);
    }
  ...
  const renderTable = (data: (PropertyType | ClientStatus)[], collection: string, setOpen: (open: boolean) => void) => (
              ...
              <TableRow key={item.id} className="hover:bg-muted/30">
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary" onClick={() => { setEditItem(item); setFormData({ name: item.name, priority_order: 1 }); setOpen(true); }}>
                  ...
                  {editingPriorityId === item.id ? (
                    ...
                    <div
                      ...
                      onClick={() => {
                        ...
                        setEditingPriorityValue(String(item.priority_order || 1));
                      }}
            ...
            <TabsContent value="property-types" className="mt-4 outline-none">
              <Card className="border-border/60 shadow-sm">
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  <Dialog open={openPropertyType} onOpenChange={(open) => { setOpenPropertyType(open); if(!open) { setEditItem(null); setFormData({name:'', priority_order: 1}); }}}>
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4 py-4">
                        <div className="space-y-2">
                          <Label>{t('Name')}</Label>
                          <Input value={formData.name} onChange={e => setFormData({name: e.target.value, priority_order: formData.priority_order})} placeholder={t('e.g. Villa')} />
                        </div>
                      </div>
                      <DialogFooter>
