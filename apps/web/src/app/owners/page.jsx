...
import OwnerDetailModal from '@/components/OwnerDetailModal.jsx';
import FilterPanel from '@/components/FilterPanel.jsx';
import FilterChips from '@/components/FilterChips.jsx';
import EmployeeReassignmentModal from '@/components/EmployeeReassignmentModal.jsx';
...
import { useOwnerStatusBadge } from '@/hooks/useOwnerStatusBadge.js';
...
const OwnerCard = ({ owner, employees, onEdit, onDelete, onClick, isSelected, onToggleSelect, companyId }) => {
...
const OwnersPage = () => {
  ...
  const [formData, setFormData] = useState({ name: '', phone: '', country: 'UAE', assigned_employee_id: '', marketing_channel: '' });
  ...
  const fetchData = async () => {
    ...
    try {
      const [statRes, empRes, mchRes] = await Promise.all([
        pb.collection('owner_statuses').getList(1, 100, { filter: `company_id="${company.id}"`, $autoCancel: false }),
      ...
      if (filterState.statusId) {
        const histMatches = await pb.collection('owner_status_history').getFullList({
      ...
      const data = await pb.collection('owners').getFullList({
        filter: finalFilter,
        $autoCancel: false,
        sort: '-created'
      });
  ...
  const handleExportCSV = async () => {
    ...
    try {
      const histories = await pb.collection('owner_status_history').getFullList({
      ...
      const columns = [
        o => o.name || '',
        o => o.phone || '',
        o => employees.find(e => e.id === o.assigned_employee_id)?.name || 'غير مسند',
        o => o.marketing_channel || '',
        o => statusMap[o.id]?.expand?.status_id?.name || 'لا يوجد',
        o => propsCountMap[o.id] || 0,
        o => statusMap[o.id] ? format(new Date(statusMap[o.id].created), 'dd/MM/yyyy') : 'لا يوجد',
        o => {
        ...
        o => o.created ? format(new Date(o.created), 'dd/MM/yyyy') : ''
      ];
  ...
  const handleBulkReassign = async (targetEmployeeId) => {
  ...
  const toggleSelectOwner = (id, checked) => {
    if (checked) {
