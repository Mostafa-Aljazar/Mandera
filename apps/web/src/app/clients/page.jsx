...
const ClientsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [emp, prop, stat, mchRes] = await Promise.all([
        ...
        pb.collection('marketing_channels').getFullList({ filter: `company_id="${company.id}"`, $autoCancel: false })
      ]);
      ...
      if (filterState.statusId) {
        fetchPromises.push(
          pb.collection('client_status_history').getFullList({
      ...
      const [fetchedClients, allHistories] = await Promise.all(fetchPromises);
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const histories = await pb.collection('client_status_history').getFullList({
        filter: `company_id="${company.id}"`,
        sort: '-created',
        expand: 'status_id',
        $autoCancel: false
      ...
      const columns = [
        ...
        c => c.phone || '',
        c => c.email || 'N/A',
        c => c.expand?.employee_id?.name || c.expand?.employee_id?.firstName || c.expand?.employee_id?.email || 'غير مسند',
        c => c.marketing_channel || '',
        c => statusMap[c.id]?.expand?.status_id?.name || 'بدون حالة',
        c => c.created ? format(new Date(c.created), 'dd/MM/yyyy') : ''
      ];
  ...
  const handleSaveClientInfo = async (formData) => {
    ...
    try {
      ...
      if (activeClient?.id) {
        try {
          savedClient = await pb.collection('clients').update(activeClient.id, payload, { expand: 'employee_id,interested_properties', $autoCancel: false });
        ...
        try {
          savedClient = await pb.collection('clients').create(payload, { expand: 'employee_id,interested_properties', $autoCancel: false });
          console.log('Success:', savedClient.id);
  ...
  const handleBulkAssign = async ({ employeeId, statusId }) => {
    try {
      ...
      for (const client of selectedClientsData) {
        ...
        if (!resolvedStatusId) {
          try {
            const latestHistory = await pb.collection('client_status_history').getFirstListItem(`client_id="${client.id}"`, {
              sort: '-created',
              $autoCancel:false
            });
        ...
        if (resolvedStatusId) {
          const oldEmpName = client.expand?.employee_id?.name || client.expand?.employee_id?.firstName || client.expand?.employee_id?.email || 'Unassigned';
          const newEmpName = newEmployee?.name || newEmployee?.firstName || newEmployee?.email || 'Unknown';
          const adminName = currentUser.name || currentUser.firstName || currentUser.email || 'Admin';
              ...
              {currentUser?.role === 'company_super_admin' && (
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    {employees.map(emp => (
                      <SelectItem key={emp.id} value={emp.id}>{emp.name || emp.firstName || emp.email}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              )}
                      ...
                      <div className="flex items-center gap-3 mb-4 mt-2">
                        ...
                        <div className="pr-12">
                          ...
                          <div className="flex items-center text-xs text-muted-foreground gap-1">
                            <User className="h-3 w-3" /> {t('Agent')}: {c.expand?.employee_id?.name || c.expand?.employee_id?.firstName || t('Unassigned')}
                          </div>
                        </div>
                      </div>
