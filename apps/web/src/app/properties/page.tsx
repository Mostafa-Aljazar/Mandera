...
import type { Property as PropertyBase, PropertyType, Owner, CompanyEmployee, MarketingChannelRecord } from '../../../types/pocketbase.types';
...
const PropertiesPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [props, typ, own, emp, allAreas] = await Promise.all([
        pb.collection('properties').getFullList<Property>({ filter: filterStr, expand: 'type,employee_id,area_district,owner_id', sort: '-created', $autoCancel: false }),
        pb.collection('property_types').getFullList<PropertyType>({ filter: `company_id = "${company.id}"`, $autoCancel: false }),
        pb.collection('owners').getFullList<Owner>({ filter: `company_id = "${company.id}"`, $autoCancel: false }),
        pb.collection('company_employees').getFullList<CompanyEmployee>({ filter: `companyId = "${company.id}"`, $autoCancel: false }),
        pb.collection('areas_districts').getFullList<AreaDistrict>({ filter: `company_id = "${company.id}"`, sort: 'name', $autoCancel: false })
      ]);
  ...
  const fetchAreas = async (emirate: string) => {
    ...
    try {
      const res = await pb.collection('areas_districts').getList<AreaDistrict>(1, 50, {
  ...
  const handleRemoveFilter = (key: keyof PropertyFilterState, valueToRemove?: string) => {
    setFilterState(prev => {
      const newState: any = { ...prev };
      if (key === 'areas' && valueToRemove) {
        newState.areas = newState.areas.filter((id: string) => id !== valueToRemove);
      } else {
        newState[key] = null;
      }
      return newState;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if(formData.land_area) payload.append('land_area', String(Number(formData.land_area)));
      if(formData.building_area) payload.append('building_area', String(Number(formData.building_area)));
      if(formData.commission_percentage) payload.append('commission_percentage', String(Number(formData.commission_percentage)));
      ...
      if (editId) {
        let updatedProp;
        try {
          updatedProp = await pb.collection('properties').update<Property>(editId, payload, { $autoCancel: false });
        } catch (updateErr) {
          throw updateErr;
        }
        ...
        let newProp;
        try {
          newProp = await pb.collection('properties').create<Property>(payload, { $autoCancel: false });
        } catch (createErr) {
          throw createErr;
        }
        ...
        if (newProp && newProp.id) {
          try {
            await pb.collection('property_status_history').create({
              ...
              created_by_name: currentUser?.name || currentUser?.email || 'Unknown User',
  ...
  const handleQuickStatusChange = async (propertyId: string, newStatus: string) => {
    ...
    try {
      const updatedProp = await pb.collection('properties').update<Property>(propertyId, { status: newStatus }, { $autoCancel: false });
      if (updatedProp && updatedProp.id) {
        try {
          await pb.collection('property_status_history').create({
