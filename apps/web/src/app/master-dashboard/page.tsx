...
const MasterDashboardPage = () => {
  ...
  const { count: employeeCount, loading: loadingEmployees } = useEmployeeCount(undefined);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const companies = await pb.collection('companies').getFullList<Company>({ $autoCancel: false });
