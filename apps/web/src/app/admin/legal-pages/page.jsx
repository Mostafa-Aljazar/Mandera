...
const LegalPagesManagementPage = () => {
  ...
  const [privacyData, setPrivacyData] = useState({ id: null, title: '', content: '', updated: null });
  const [termsData, setTermsData] = useState({ id: null, title: '', content: '', updated: null });
  ...
  const fetchPages = async () => {
    try {
      const records = await pb.collection('legal_pages').getFullList({
        $autoCancel: false
      });
  ...
  const handleSave = async (type, data) => {
    ...
    const stateKey = isPrivacy ? 'privacy' : 'terms';
    ...
    try {
      ...
      try {
        await pb.collection('legal_pages').update(data.id, {
          ...payload,
        ...
        if (updateErr.status === 400) {
          ...
          await pb.collection('legal_pages').update(data.id, payload, { $autoCancel: false });
        } else {
          throw updateErr;
        }
      }
