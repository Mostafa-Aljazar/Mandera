...
const EmployeesPage = () => {
  ...
  const [activeEmployee, setActiveEmployee] = useState<CompanyEmployee | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Deletion Dialog State
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [selectedEmployeeForDeletion, setSelectedEmployeeForDeletion] = useState<CompanyEmployee | null>(null);
  ...
  const fetchEmployees = async () => {
    ...
    try {
      const data = await pb.collection('company_employees').getFullList<CompanyEmployee>({
  ...
  const handleSaveEmployee = async (formData: EmployeeFormData) => {
    ...
    try {
      if (activeEmployee?.id) {
        // Update existing
        await pb.collection('company_employees').update(activeEmployee.id, {
          name: formData.name,
        ...
        if (activeEmployee.employeeId) {
          ...
          await pb.collection('employees').update(activeEmployee.employeeId, {
            firstName: firstName || '',
  ...
  return (
    <>
      <Helmet>
        <title>{t('platformName')} - {t('Employees')}</title>
      </Helmet>
      <CompanyAdminHeader />
      ...
      <main className="min-h-[calc(100vh-80px)] bg-muted/20 py-8">
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
            <div>
              <h1 className="text-3xl font-bold tracking-tight font-outfit">{t('Team Directory')}</h1>
              <p className="text-muted-foreground mt-1">{t('Manage your company employees and their access roles.')}</p>
            </div>

            <Button onClick={() => handleOpenModal(null)} className="gap-2 shadow-sm rounded-xl">
              <Plus className="h-4 w-4" /> {t('Add New Employee')}
            </Button>
          </div>
                ...
                {employees.length === 0 ? (
                  ...
                  employees.map(emp => (
                    <TableRow key={emp.id} className="hover:bg-muted/30 transition-colors">
                      ...
                      <TableCell className="text-muted-foreground">{t('N/A')}</TableCell>
                      <TableCell>
                        <Badge variant={emp.role === 'company_super_admin' ? 'default' : 'secondary'} className="font-medium">
                          {emp.role === 'company_super_admin' ? t('Super Admin') : t('Employee')}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <Badge variant={emp.expand?.employeeId?.disabled ? 'destructive' : 'outline'} className={!emp.expand?.employeeId?.disabled ? 'bg-emerald-500/10 text-emerald-600 border-emerald-200' : ''}>
                          {emp.expand?.employeeId?.disabled ? t('Inactive') : t('Active')}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="icon" onClick={() => handleOpenModal(emp)} className="h-8 w-8 text-muted-foreground hover:text-primary transition-colors">
                            <Edit2 className="h-4 w-4" />
                          </Button>
                          {emp.id !== currentUser?.id && (
                            <Button variant="ghost" size="icon" onClick={() => initiateDelete(emp)} className="h-8 w-8 text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors">
