...
const StatusHistoryDisplay = ({ entityType, entityId, refreshTrigger }) => {
  ...
  const fetchHistory = useCallback(async () => {
    if (!entityId || !company?.id) return;
    setIsLoading(true);
    try {
      const collectionName = `${entityType}_status_history`;
      const entityIdField = entityType === 'client' ? 'client_id' : entityType === 'owner' ? 'owner_id' : 'property_id';
      
      // Expand status_id if the schema uses relation for status
      const expandRelations = entityType !== 'property' ? 'status_id' : '';

      const res = await pb.collection(collectionName).getList(1, 50, {
        filter: `company_id="${company.id}" && ${entityIdField}="${entityId}"`,
        sort: '-created',
  ...
  }, [entityType, entityId, company?.id]);

  useEffect(() => {
    fetchHistory();
  }, [fetchHistory, refreshTrigger]);

  const handleDelete = async (recordId, creatorId) => {
  ...
  const renderStatusBadge = (h) => {
    ...
    if (entityType === 'property') {
      ...
      statusText = h.expand?.status_id?.name || h.status || statusText;
    }
              ...
              <div key={h.id} className="relative pl-6 rtl:pl-0 rtl:pr-6 pb-1 border-l-2 rtl:border-l-0 rtl:border-r-2 border-muted last:border-transparent last:pb-0 group">
                ...
                <div className="bg-muted/20 rounded-xl p-4 border border-border/50 shadow-sm hover:border-primary/20 transition-colors">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground border-t border-border/40 pt-2 mt-1">
                    ...
                    <span>
                      {/* Arabic specific formatting requested */}
