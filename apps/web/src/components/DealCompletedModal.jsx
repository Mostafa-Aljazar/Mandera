...
const DealCompletedModal = ({ isOpen, onClose, property, onSuccess }) => {
    ...
    try {
      ...
      if (updatedProp && updatedProp.id) {
        ...
        try {
          ...
          console.error('PocketBase Error (revenues):', revErr.status, revErr.message, revErr.response);
        ...
        try {
          const historyData = {
            ...
            created_by_name: pb.authStore.model.name || pb.authStore.model.email || 'System',
            company_id: company.id
          };
          ...
          console.error('PocketBase Error (status_history):', histErr.status, histErr.message, histErr.response);
      ...
      toast.error(err.message || t('Error saving deal completion.'));
    } finally {
      setSubmitting(false);
    }
  };
