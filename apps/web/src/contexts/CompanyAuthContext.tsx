...
export const CompanyAuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      if (
        ...
        model.name = (model.name || model.firstName || model.email || 'Unknown User') as string;
        ...
        try {
          if (model.companyId) {
            const companyData = await pb.collection('companies').getOne(model.companyId as string, { $autoCancel: false });
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      // Ensure name is always populated correctly
      userRecord.name = (userRecord.name || userRecord.firstName || userRecord.email || 'Unknown User') as string;

      if (!userRecord.companyId) {
        throw new Error('Your user account is not associated with any company.');
      }

      const companyData = await pb.collection('companies').getOne(userRecord.companyId as string, { $autoCancel: false }) as unknown as Company;
      ...
      pb.authStore.clear();
      throw new Error((error as Error).message || 'An error occurred during login validation.');
    }
  };
